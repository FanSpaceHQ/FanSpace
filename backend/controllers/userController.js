...
const {update_fields,required_fields}=require('./../constants/userConstants')
...
const addUserToEvent=async(req,res)=>{ //take in three params, the user we're changing, the event we're changing their relation to, and which field we're chaning (interested, selling, going)
  ...
  if(!event||!uid||field!='following'||field!='interested'||field!='going') //check for all parameters
  ...
  if(!userdoc.exists)
    ...
    if(!eventdoc.exists){ //if event doc doesn't exist, we create it with the name uid 
  ...
  const objectTest=async(req,res)=>{
    ...
    console.log(events)
    database.collection('users').doc(uid).set(events).then(()=>{
      res.status(200)
    })
  }
module.exports={
